"""
AI Market Insights - Hardened News Pipeline (SRE Audit Fixes)
Production-ready Cloud Function with cost optimization, data integrity, and reliability fixes
"""
...
class ArticleModel(BaseModel):
    """Pydantic model for individual article validation"""
...
class NewsPipelineHardened:
    """
    Hardened news pipeline with SRE fixes:
    ...
    def _load_existing_urls(self) -> None:
        ...
        This replaces the individual check_duplicate() calls with a single batch operation
        """
        try:
            ...
            # Continue with empty set rather than crashing
            self.existing_urls = set()
    
    def _build_combined_prompt(self) -> str:
        """
        SRE Fix #1: Single combined prompt for search + analysis
        This eliminates the N+1 API call pattern and reduces costs by ~95%
        """
    ...
    def fetch_and_analyze_articles(self) -> List[Dict[str, Any]]:
        ...
        This reduces API calls from N+1 to 1, dramatically reducing costs and latency
        """
        try:
            ...
            response = self.model.generate_content(
                ...
                generation_config={
                    "temperature": 0.2,
                    "max_output_tokens": 8192,  # Increased for multiple articles
    ...
    def _parse_and_validate_response(self, response_text: str) -> List[Dict[str, Any]]:
        ...
        This prevents crashes from malformed AI responses and ensures data integrity
        """
        try:
            # Clean response text
    ...
    def _filter_new_articles(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        This replaces N+1 database queries with O(1) set lookups
        """
        new_articles = []
    ...
    def run(self) -> Dict[str, Any]:
        """
        Main pipeline execution with all SRE fixes applied
        """
        stats = {
            "fetched": 0,
            "new_articles": 0,
        ...
        try:
            logger.info("Starting hardened news pipeline...")
            
            # SRE Fix #2: Load existing URLs to avoid N+1 queries
            self._load_existing_urls()
            
            # SRE Fix #1: Single API call for search + analysis
            ...
            # SRE Fix #2: Efficient duplicate filtering
            new_articles = self._filter_new_articles(articles)
            stats["new_articles"] = len(new_articles)
            stats["analyzed"] = len(new_articles)  # All articles were analyzed in the single call
            
            # Batch save to Firestore
            saved_count = self._batch_save_articles(new_articles)
            stats["saved"] = saved_count
            
            # Update existing URLs cache for potential future runs
...
def news_pipeline_hardened(request):
    """
    Hardened Cloud Function entry point
    SRE Fix #4: Configured for concurrency=1 to prevent race conditions
    """
    logger.info("Starting hardened news pipeline...")
    ...
    logger.info(f"Hardened news pipeline completed: {stats}")
